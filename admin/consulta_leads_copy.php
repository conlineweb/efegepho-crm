...
function getDondeNosConocioLabel($lead)
{
    ...
    if ($howRaw === '' && in_array($tablaOrigen, ['wedding_planners', 'eventos_wp', 'wp_eventos_afianzados', 'wp_citas_leads'], true)) {
        $howRaw = '1';
    }
...
function getHearAboutUsLabel($lead)
{
    $hearRaw = trim((string)($lead['hear_about_us'] ?? ''));
    ...
    return ($hearRaw !== '') ? ($hearMap[$hearRaw] ?? $hearRaw) : '';
}
...
foreach ($leadIdsByTable as $t => $ids) {
    ...
    if (trim($idsList) === '')
        continue;
    $sql = "SELECT id, original_lead_id, cliente, how_did_you_meet, hear_about_us, engagement, first_contact_channel, referrer_url FROM contact_form WHERE LOWER(tabla_origen) = LOWER('" . $safeTable . "') AND original_lead_id IN (" . $idsList . ")";
...
foreach ($filteredLeads as $lead) {
    ...
    $knownUsLabel = getKnownUsBadgeLabel($lead['how_long_known_us'] ?? '');
...
<head>
    ...
    <style>
        ...
        th[data-column="exactamente_como_llego"],
        td[data-column="exactamente_como_llego"],
        ...
        <div class="report-overview-grid">
            ...
            <div class="report-breakdown-grid">
                <article class="report-breakdown-card">
                    ...
                    <div class="report-card-note">Desglose de cómo entraron los leads del periodo.</div>
                ...
                <article class="report-breakdown-card">
                    ...
                    <div class="report-card-note">Desglose del origen declarado por cada lead.</div>
                ...
                <article class="report-breakdown-card">
                    ...
                    <div class="report-card-note">Desglose según la antigüedad declarada por el lead.</div>
                ...
                <article class="report-breakdown-card">
                    ...
                    <div class="report-card-note">Desglose por campaña de origen del lead.</div>
            ...
            <table id="leadsTable" class="efege-table table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th data-column="exactamente_como_llego"><div class="th-flex-label"><span class="th-compact-label">Exactamente como llegó</span><i class="filter-icon bi bi-funnel" data-column="exactamente_como_llego"></i></div></th>
                                    ...
                                    <?php if ($tipoUsuario != "4"): ?>
                                        <th class="assign-col" data-column="asignado">Vendedor</th>
                            ...
                            <tbody>
                                <?php foreach ($filteredLeads as $lead): ?>
                                    ...
                                    if ($_fcc === 'leadform') {
                                        $lead['how_did_you_meet'] = '3'; // New Market
                                        $lead['hear_about_us']    = '1'; // Meta Ads
                                    } elseif ($_fcc === 'ig') {
                                        if ($_igOrganico) {
                                            ...
                                            $lead['hear_about_us']    = '1';
                                        }
                                    } elseif (in_array($_fcc, ['website', 'mail', 'email', 'whatsapp'], true)) {
                                        $lead['how_did_you_meet'] = '';
                                        $lead['hear_about_us']    = '';
                                    } elseif (empty($lead['how_did_you_meet']) && in_array($_tablaOLower, ['wedding_planners', 'eventos_wp', 'wp_eventos_afianzados', 'wp_citas_leads'], true)) {
                                    ...
                                    $hearAboutUsLabel = getHearAboutUsLabel($lead);
                                    $exactSourceLabel = $hearAboutUsLabel !== '' ? $hearAboutUsLabel : 'Not asked';
                                    $exactSourceDisplayLabel = $exactSourceLabel === 'Meta Ads — anuncio en Instagram / Facebook'
                                        ? 'Meta Ads'
                                        : $exactSourceLabel;
                                    ...
                                    <tr id="lead-row-<?php echo $lead['id']; ?>-<?php echo htmlspecialchars($lead['tabla_origen'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                        ...
                                        <td data-column="exactamente_como_llego"><span class="badge-source" title="<?php echo htmlspecialchars($exactSourceLabel, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($exactSourceDisplayLabel, ENT_QUOTES, 'UTF-8'); ?></span></td>
                                        <td data-column="campana"><span class="badge-campaign"><?php echo htmlspecialchars($campaignDisplayLabel, ENT_QUOTES, 'UTF-8'); ?></span></td>
                                        ...
                                        <td data-column="cuando_se_casa"><span class="td-inline-muted"><?php echo htmlspecialchars(formatLeadDate($weddingDateDisplay), ENT_QUOTES, 'UTF-8'); ?></span></td>
...
<script>
                    ...
                    if (resp.success) {
                        ...
                        if (resp.mode === 'update') {
                            ...
                            if ($row.length) {
                                ...
                                $row.find('td[data-column="campana"]').text(jsFormatCampaignDisplay(origen || ''));
    ...
    function renderBreakdownList(targetSelector, counts, totalCount, emptyLabel) {
        ...
        var html = entries.map(function (entry) {
            ...
            } else if (displayLabel === 'Sin dato') {
                displayLabel = 'Not asked';
            }
