...
function resolveLeadStatus($tableName, $contactFormRow, $appointmentRow)
{
...
foreach ($leadIdsByTable as $t => $ids) {
    foreach ($ids as $lid) {
        ...
        if (isset($contactFormByLead[$key])) {
            ...
            $leadStatusMap[$key] = resolveLeadStatus($t, $cf, $appointment);
        }
    }
}
...
foreach ($filteredLeads as $lead) {
    ...
    if ($raw === '' && $st === 'muerto') {
        ...
        $contactLabel = normalizeFirstContactChannelLabel($raw);
    }
...
// Agregar data para gráfica "Desde cuándo nos conoce"
$knownUsCounts = [];
foreach ($filteredLeads as $lead) {
...
<head>
    ...
    <style>
        ...
        /* Hacer más compactas algunas columnas informativas */
...
/* ── EFEGE TABLE ── */
.efege-table-scroll { overflow: visible; width: 100%; }
/* DataTables scrollX: solo el cuerpo de la tabla hace scroll horizontal */
.efege-table-scroll .dataTables_scrollBody {
  scrollbar-width: thin;
  scrollbar-color: var(--border, #e2e8f0) transparent;
}
.efege-table-scroll .dataTables_scrollBody::-webkit-scrollbar { height: 6px; }
.efege-table-scroll .dataTables_scrollBody::-webkit-scrollbar-track { background: transparent; }
.efege-table-scroll .dataTables_scrollBody::-webkit-scrollbar-thumb { background: var(--border, #e2e8f0); border-radius: 3px; }
...
.rlm-field select { width: 100%; border: 1px solid #ddd; border-radius: 8px; padding: 9px 12px; font-size: 0.9rem; color: #333; outline: none; transition: border-color 0.2s; background: #fff; }
.rlm-field input:focus, .rlm-field select:focus { border-color: #464646; }
...
#leadHowLongKnownUsGroup .rlm-choice-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); }
.rlm-choice-card {
            ...
            <table id="leadsTable" class="efege-table table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <?php if ($tipoUsuario != "4"): ?>
                                        <th class="assign-col" data-column="asignado">Asignar Pre Q</th>
                            ...
                            <tbody>
                                <?php foreach ($filteredLeads as $lead): ?>
                                    ...
                                    // Enriquecer con campos de contact_form si el lead no los trae directamente
                                    if (isset($contactFormByLead[$rowStatusKey])) {
                                        $cfExtra = $contactFormByLead[$rowStatusKey];
                                        foreach (['how_did_you_meet', 'hear_about_us', 'engagement', 'first_contact_channel'] as $_cfField) {
                                    ...
                                    $knownUsDisplay = firstNonEmptyValue($lead['how_long_known_us'] ?? ''); ?>
                                    <?php $isManualWP = (isset($lead['tabla_origen']) && $lead['tabla_origen'] === 'wedding_planners'); ?>
                                    <tr id="lead-row-<?php echo $lead['id']; ?>-<?php echo htmlspecialchars($lead['tabla_origen'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                        ...
                                        <td data-column="boda"><?php echo htmlspecialchars($weddingLocationDisplay !== '' ? $weddingLocationDisplay : '—', ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td data-column="cuando_se_casa"><?php echo htmlspecialchars(formatLeadDate($weddingDateDisplay), ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td data-column="ciudad_origen"><?php echo htmlspecialchars($leadCityDisplay !== '' ? $leadCityDisplay : '—', ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td data-column="fecha">
                                            <?php echo htmlspecialchars(formatCreatedTime($lead['created_time'] ?? '')); ?>
                                        </td>
                                        <td data-column="metodo_contacto"><?php echo htmlspecialchars($contactMethodDisplay !== '' ? $contactMethodDisplay : '—', ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td data-column="desde_conoce"><?php echo htmlspecialchars($knownUsDisplay !== '' ? $knownUsDisplay : '—', ENT_QUOTES, 'UTF-8'); ?></td>
                                        <?php
                                        ...
                                        <?php if ($tipoUsuario != "4"): ?>
                                            ...
                                            <td class="gap-2">
                                                <?php if ($lead['tabla_origen'] === 'organic_leads'): ?>
                                                    <?php if (isset($lead['lead_status']) && $lead['lead_status'] === 'manual'): ?>
                                                        <button class="mb-2 btn btn-sm btn-editar-lead" title="Editar Lead Manual"
                                                            style="background-color: #c5a028; color: white; border: 1px solid #c5a028; margin-right: 5px;"
                                                            onclick='editarLeadManual(<?php echo json_encode($lead); ?>)'><i
                                                    ...
                                                    <?php else: ?>
                                                        ...
                                                        <button class="mb-2 btn btn-info btn-sm" title="Ver Link del Formulario"
                                                        ...
                                                        <?php if (!empty($cleanPhone) && strlen($cleanPhone) >= 8): ?>
                                                            <?php if (isset($lead['whatsapp_enviado']) && $lead['whatsapp_enviado'] == 1): ?>
                                                                <button class="mb-2 btn btn-success btn-sm whatsapp-btn"
                                                                    title="WhatsApp enviado"
                                                                    style="background-color:#25D366;color:white;border: 1px solid #25D366; margin-right:5px;"><i
                                                                        class="bi bi-whatsapp"></i> WhatsApp<br /><small
                                                                        class="text-muted">enviado</small></button>
                                                            <?php else: ?>
                                                                <button class="mb-2 btn btn-success btn-sm whatsapp-btn"
                                                                    title="Enviar por WhatsApp"
                                                                    style="background-color:#25D366;color:white;border: 1px solid #25D366; margin-right:5px;"
                                                                    onclick='sendWhatsApp(this, <?php echo json_encode($lead['tabla_origen']); ?>, <?php echo $lead['id']; ?>, <?php echo json_encode($cleanPhone); ?>, <?php echo json_encode($lead['full_name'] ?? ''); ?>)'><i
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formLeadManual">
                        ...
                        <div class="rlm-section">
                            ...
                            <div class="rlm-grid-2">
                                ...
                                <div class="rlm-field">
                                    ...
                                    <span class="rlm-error-msg" id="err-leadTelefono">El teléfono debe tener 10 dígitos (solo números)</span>
                                </div>
                            </div>
                        ...
                        <!-- Sección 2: Primer contacto, origen y medio -->
                        <div class="rlm-section">
                            <div class="rlm-section-title">
                            ...
                            <div class="rlm-field">
                                ...
                                <div class="rlm-choice-group" id="leadHowLongKnownUsGroup">
                                    <div class="rlm-choice-grid">
                                        <button type="button" class="rlm-choice-card" data-target="#leadHowLongKnownUs" data-value="Less than 3 months">
                                            <span class="rlm-choice-card-title">Menos de 3 meses</span>
                                            <span class="rlm-choice-card-desc">Nos conociste recientemente.</span>
                                        </button>
                                        <button type="button" class="rlm-choice-card" data-target="#leadHowLongKnownUs" data-value="Between 3 months and 1 year">
                                            <span class="rlm-choice-card-title">Entre 3 meses y 1 año</span>
                                            <span class="rlm-choice-card-desc">Ya tenías un tiempo siguiéndonos.</span>
                                        </button>
                                        <button type="button" class="rlm-choice-card" data-target="#leadHowLongKnownUs" data-value="More than 1 year">
                                            <span class="rlm-choice-card-title">Más de 1 año</span>
                                            <span class="rlm-choice-card-desc">Nos conocías desde hace bastante tiempo.</span>
                                        </button>
                                        <button type="button" class="rlm-choice-card" data-target="#leadHowLongKnownUs" data-value="Not asked">
                                            <span class="rlm-choice-card-title">No se preguntó</span>
                                            <span class="rlm-choice-card-desc">Úsalo cuando esa pregunta no se haya hecho al lead.</span>
                                        </button>
                                    </div>
                                </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#leadsTable').DataTable({
            ...
            // scrollX: DataTables maneja el scroll horizontal internamente
            // (search y paginación quedan fuera del área de scroll)
            scrollX: true,
            // Contenedor fijo de 1200px: el scroll ocurre solo dentro del contenedor
            scrollY: '800px',
            scrollCollapse: true,
            // drawCallback sigue siendo útil para refrescar botones dinámicos
            drawCallback: function () {
                waRefreshConversationButtons(document);
                updateDynamicCards(); // Actualizar cards cuando cambie la tabla
            },
            columnDefs: [
    ...
    function verMas(lead) {
        ...
        if (tablaOrigen === 'organic_leads') {
            ...
            if (lead['lead_status'] === 'manual') {
                actionsHtml += '<button class="mb-1 btn btn-warning btn-sm btn-editar-lead" title="Editar Lead Manual" style="background-color:#c5a028;color:white;border:1px solid #c5a028;" onclick=\'editarLeadManualAndCloseModal(' + JSON.stringify(lead) + ')\'><i class="bi bi-pencil"></i> Editar Lead</button>';
            }
    ...
    $(document).ready(function () {
        ...
        window.jsFormatCreatedTime = function jsFormatCreatedTime(dtStr) {
            ...
            return d.getDate() + ' de ' + months[d.getMonth()] + ' de ' + d.getFullYear() + ' a las ' + h12 + ':' + mm + ' ' + ampm;
        }
        window.jsFormatLeadDate = function jsFormatLeadDate(dtStr) {
            if (!dtStr) return '—';
        ...
        window.editarLeadManual = function editarLeadManual(lead) {
            ...
            $('#leadWeddingDate').val(lead['wedding_date'] ? String(lead['wedding_date']).substring(0, 10) : '');
        ...
        $('#btnGuardarLeadManual').on('click', function () {
            ...
            if (phoneDigits.length > 0) {
                ...
                // Permitir teléfono local de 10 dígitos o número completo con código de país seleccionado
                if (!normalizedPhone.valid) {
                    $('#leadTelefono').addClass('is-invalid');
                    isValid = false;
                    invalidFields.push('Teléfono (10 dígitos locales)');
                }
            }
                    ...
                    if (resp.success) {
                        ...
                        if (resp.mode === 'update') {
                            ...
                            if ($row.length) {
                                ...
                                $row.find('td[data-column="cuando_se_casa"]').text(jsFormatLeadDate(updatedWeddingDate));
                                $row.find('td[data-column="ciudad_origen"]').text(updatedCity || '—');
                                $row.find('td[data-column="fecha"]').text(jsFormatCreatedTime(updatedCreatedTime));
                                $row.find('td[data-column="metodo_contacto"]').text(jsNormalizeContactChannel(firstContactChannel));
                                $row.find('td[data-column="desde_conoce"]').text(howLongKnownUs || '—');
                                // Actualizar atributos del tr
                                $row.attr('data-full-name', nombre).attr('data-email', correo);
                                ...
                                $row.find('.btn-editar-lead').attr('onclick',
                                    'editarLeadManual(' + JSON.stringify(updatedLead) + ')');
    ...
    function getUniqueColumnValues(columnName) {
        ...
        $('#leadsTable tbody tr').each(function () {
            ...
            if (cell.length > 0) {
                ...
                if (text !== '' && text !== '—') {
                    if (columnName === 'metodo_contacto') {
                        text = jsNormalizeContactChannel(text);
                    }
                    values.add(text);
                }
            }
    ...
    function createFilterDropdown(columnName, iconElement) {
        ...
        var clearBtn = $('<button type="button" class="filter-btn btn btn-sm btn-secondary">Limpiar</button>');
        var applyBtn = $('<button type="button" class="filter-btn btn btn-sm btn-primary">Aplicar</button>');
        ...
        $('#leadsTable tbody tr').each(function () {
            ...
            for (var columnName in columnFilters) {
                ...
                if (cell.length > 0) {
                    ...
                    if (columnName === 'metodo_contacto') {
                        cellValue = jsNormalizeContactChannel(cellValue);
                    }
