            ...
            if (!empty($formName) && $origId > 0) {
                ...
                if ($checkTable && $checkTable->num_rows > 0) {
                    ...
                    if ($leadRes && $leadRes->num_rows > 0) {
                        ...
                        // Dar prioridad al first_contact_channel del lead original sobre el de contact_form,
                        // ya que contact_form puede tener un valor incorrecto (ej: 'whatsapp' para leads de IG).
                        if (!empty($leadRow['first_contact_channel'])) {
                            $merged['first_contact_channel'] = trim($leadRow['first_contact_channel']);
                        }
...
if (!function_exists('getHearAboutUsLabel')) {
...
if (!function_exists('formatCampaignDisplayLabel')) {
    function formatCampaignDisplayLabel($value) {
        $normalized = trim((string) $value);
        if ($normalized === '') {
            return '—';
        }

        if (preg_match('/^(c\d+)\b/i', $normalized, $matches)) {
...
foreach ($displayLeads as $lead) {
    $label = getContactChannelBadgeLabel(normalizeFirstContactChannelLabel($lead['first_contact_channel'] ?? ''));
...
$postCampaignCounts = [];
foreach ($displayLeads as $lead) {
    $cn = trim((string)($lead['campaign_name'] ?? ''));
    if ($cn === '') $cn = '—';
...
$postWhereKnowCounts = ['Wedding Planner' => 0, 'Community' => 0, 'New Market' => 0, 'Sin dato' => 0];
foreach ($displayLeads as $lead) {
    $howRaw = trim((string)($lead['how_did_you_meet'] ?? ''));
...
foreach ($displayLeads as $lead) {
    ...
    $eventId = intval($lead['original_lead_id'] ?? 0);
    if (intval($postEventWpAfianzadosMap[$eventId] ?? 0) === 1) {
        $postWpAfianzadosCount++;
    }
}
        ...
        <section class="reports-section">
            <div class="reports-section-header">
                <div>
                    ...
                    <h2 class="reports-section-title">Reporte de sesiones post-calificadas</h2>
            ...
            <div class="report-overview-grid">
                ...
                <div class="report-metric-grid">
                    <article class="kpi-card">
                        ...
                        <div class="kpi-sub">Sesiones agendadas dentro del periodo.</div>
                    </article>
                    ...
                    <article class="kpi-card">
                        ...
                        <div class="kpi-sub">Sesiones en estatus atendido o cliente.</div>
                    </article>
                    ...
                    <article class="kpi-card">
                        ...
                        <div class="kpi-sub" id="kpi-wp-afianzados-detail"><?php echo formatPercentage($postWpAfianzadosCount, $leadsCountFiltered); ?> del total corresponde a WP afianzados.</div>
                    </article>
                </div>
                ...
                <div class="report-breakdown-grid">
                    <article class="report-breakdown-card">
                        <div class="report-chart-header">
                            <div>
                                <h3 class="report-chart-title">Metodo de contacto</h3>
                                <p class="report-chart-subtitle">Distribucion del canal por el que se agendaron las sesiones.</p>
                            </div>
                        </div>
                    ...
                    <article class="report-breakdown-card">
                        <div class="report-chart-header">
                            <div>
                                ...
                                <p class="report-chart-subtitle">Desglose del origen capturado en el proceso post-cal.</p>
                            </div>
                        </div>
                        <div class="report-breakdown-list">
                    ...
                    <article class="report-breakdown-card">
                        <div class="report-chart-header">
                            <div>
                                <h3 class="report-chart-title">Desde cuando nos conoce</h3>
                                <p class="report-chart-subtitle">Antiguedad declarada por cada lead post-calificado.</p>
                            </div>
                        </div>
                        <div class="report-breakdown-list">
                    ...
                    <article class="report-breakdown-card">
                        <div class="report-chart-header">
                            <div>
                                ...
                                <p class="report-chart-subtitle">Desglose por campaña de origen del lead post-calificado.</p>
                            </div>
                        </div>
                        <div class="report-breakdown-list">
                    ...
                    <tbody>
                        <?php foreach ($displayLeads as $lead): ?>
                            <?php
                                ...
                                } elseif ($originCategoryLabel === 'New Market') {
                                ...
                                $hearAboutUsLabel = getComoLlegaronLabel($lead);
                                $exactSourceFullLabel = $hearAboutUsLabel !== '' ? $hearAboutUsLabel : 'Sin dato';
                                $exactSourceDisplayLabel = $exactSourceFullLabel === 'Meta Ads — anuncio en Instagram / Facebook'
                                    ? 'Meta Ads'
                                    : $exactSourceFullLabel;
                            ...
                            <tr id="lead-row-<?php echo $lead['id']; ?>-<?php echo htmlspecialchars($lead['tabla_origen']); ?>"
                                ...
                                <!-- Exactamente cómo llegó -->
                                <td data-column="exactamente_como_llego">
                                    <span class="badge-source" title="<?php echo htmlspecialchars($exactSourceFullLabel, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($exactSourceDisplayLabel, ENT_QUOTES, 'UTF-8'); ?></span>
                                </td>
