...
function enviarCorreo($correo_destino, $asunto, $titulo, $cuerpo, $despedida, $archivoAdjunto = null, $tablaOrigen = '', $leadId = '', &$mailError = null)
{
...
";
  ...
  try {
   ...
   $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // más seguro que 'starttls'
    ...
    // Adjuntar archivo PDF si se especifica
    ...
    // Capture PHPMailer error and expose to caller via reference
...
$buttonText = (!$isUsaForm) ? 'Book now' : 'Schedule now';
...
$trackUrl = 'https://citas.efegepho.com.mx/pixel/click.php?id=' . urlencode($id)
  ...
  . '&correo=1&url=' . urlencode($linkWithParams);
...
// $indexCorreo = (!$isUsaForm) ? 24 : 26; //6 español - 7 ingles
$indexCorreo = 26;
$asunto = ($indexCorreo == 24) ? "Datos recibidos" : "Received data";
...
// Ruta al archivo PDF que se adjuntará
$archivoPDF = '../files/EFEGE_packages.pdf';

// Intentar enviar el correo
  try {
  $mailError = null;
  $correoEnviado = enviarCorreo($email, $asunto, $titulo, $cuerpo, $despedida, $archivoPDF, $tablaOrigen, $id, $mailError);

  if ($correoEnviado) {
    // Actualizar la base de datos para marcar el correo como enviado
    // Marcar como enviado y registrar la fecha de envío
    $stmt = $conn->prepare("UPDATE `$tablaOrigen` SET correo_uno_enviado = 1, fecha_envio_correo_uno = NOW() WHERE id = ?");
    $stmt->bind_param("i", $id);
    ...
    if ($stmt->execute()) {
      ...
      if (isset($stmt) && $stmt->error) {
        ...
        error_log("enviar_primer_correo_lead.php DB error for lead {$id} in {$tablaOrigen}: " . $stmt->error);
      }
    ...
    if (!empty($mailError)) {
      // Include mail-specific error; be careful in production not to leak sensitive data
      $payload['mail_error'] = $mailError;
      // Log the error server-side for debugging
      error_log("enviar_primer_correo_lead.php: PHPMailer error for lead {$id} in {$tablaOrigen}: " . $mailError);
    }
    echo json_encode($payload);
  }
} catch (Exception $e) {
