...
if (!function_exists('resolveLeadBoardVendedorId')) {
...
if (!function_exists('getVendedorAsignadoLabel')) {
...
<div class="mlb-leads-tab-content mlb-leads-tab-precalificados">
<style>
...
.mlb-registro-date-cell:hover {
    color: #1d4ed8;
}

.mlb-registro-date-cell.is-empty {
    color: var(--muted);
    font-weight: 500;
}

.mlb-registro-date-hint {
    display: block;
    margin-top: 3px;
    font-size: 10px;
    font-weight: 500;
...
@media (max-width: 600px) {
    ...
    <br><form method="get" class="filter-bar" id="filterForm"><input type="hidden" name="tab" value="precalificados">
    ...
    <i class="fas fa-filter"></i> Filtrar </button>
        <a href="my_lead_board_leads.php?tab=precalificados&show_all=1" class="efege-btn">Limpiar</a>
            ...
            <table id="leadsTable" class="efege-table table table-hover">
                            ...
                            <tbody>
                                <?php foreach ($filteredLeads as $lead): ?>
                                    ...
                                    $registroRaw = trim((string) ($lead['created_time'] ?? ''));
                                    $registroTs = ($registroRaw !== '' && strtotime($registroRaw) !== false) ? (int) strtotime($registroRaw) : 0;
                                    $registroDateInput = $registroTs > 0 ? date('Y-m-d', $registroTs) : '';
                                    $registroTimeInput = $registroTs > 0 ? date('H:i', $registroTs) : '';
                                    $registroDisplay = formatCreatedTime($registroRaw);
                                    if ($registroDisplay === '') {
                                        $registroDisplay = '—';
                                    }
                                    $tablaOrigenLead = trim((string) ($lead['tabla_origen'] ?? ''));
                                    $isWpEventLead = strcasecmp($tablaOrigenLead, 'eventos_wp') === 0;
                                    $registroEditKind = $isWpEventLead ? 'eventos_wp' : 'lead';
                                    $registroContactFormId = (int) ($lead['contact_form_id'] ?? 0);
                                    $registroEventId = $isWpEventLead ? (int) ($lead['id'] ?? 0) : 0;
                                    ?>
                                    <tr id="lead-row-<?php echo $lead['id']; ?>-<?php echo htmlspecialchars($lead['tabla_origen'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                        ...
                                        <td data-column="fecha_registro"
                                            data-order="<?php echo $registroTs; ?>"
                                            <?php if ($mlbInlineFechaEditEnabled): ?>
                                            class="mlb-registro-date-cell js-edit-registro-date<?php echo $registroDisplay === '—' ? ' is-empty' : ''; ?>"
                                            title="Clic para editar fecha de registro"
                                            data-lead-kind="<?php echo htmlspecialchars($registroEditKind, ENT_QUOTES, 'UTF-8'); ?>"
                                            data-tabla-origen="<?php echo htmlspecialchars($tablaOrigenLead, ENT_QUOTES, 'UTF-8'); ?>"
                                            data-lead-id="<?php echo (int) ($lead['id'] ?? 0); ?>"
                                            data-contact-form-id="<?php echo $registroContactFormId; ?>"
                                            data-event-id="<?php echo $registroEventId; ?>"
                                            data-fecha="<?php echo htmlspecialchars($registroDateInput, ENT_QUOTES, 'UTF-8'); ?>"
                                            data-hora="<?php echo htmlspecialchars($registroTimeInput, ENT_QUOTES, 'UTF-8'); ?>"
                                            data-lead-name="<?php echo htmlspecialchars($nameDisplay['primary'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                            <?php endif; ?>>
                                            <span class="td-inline-muted"><?php echo htmlspecialchars($registroDisplay, ENT_QUOTES, 'UTF-8'); ?></span>
                                            <?php if ($mlbInlineFechaEditEnabled): ?>
                                            <span class="mlb-registro-date-hint">Editar</span>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        $fechaRegistroDisplay = $registroDisplay;
                                        ?>
...
<div class="modal fade" id="editRegistroDateModal" tabindex="-1" aria-labelledby="editRegistroDateModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editRegistroDateModalLabel">Editar fecha de registro</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
            <div class="modal-body">
                <p class="mb-2" style="font-size:13px;color:var(--muted);">
                    <strong id="editRegistroDateLeadName" style="color:var(--ink);"></strong>
                </p>
                <p class="mb-3" id="editRegistroDateContext" style="font-size:12px;color:var(--muted);"></p>
                <form id="editRegistroDateForm">
                    <input type="hidden" id="editRegistroDateKind" value="lead">
                    <input type="hidden" id="editRegistroDateTablaOrigen" value="">
                    <input type="hidden" id="editRegistroDateLeadId" value="0">
                    <input type="hidden" id="editRegistroDateContactFormId" value="0">
                    <input type="hidden" id="editRegistroDateEventId" value="0">
                    <div class="mb-3">
                        <label for="editRegistroDateFecha" class="form-label">Fecha de registro</label>
                        <input type="date" class="form-control" id="editRegistroDateFecha" required>
                    </div>
                    <div class="mb-0">
                        <label for="editRegistroDateHora" class="form-label">Hora</label>
                        <input type="time" class="form-control" id="editRegistroDateHora">
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <button type="button" class="btn btn-primary" id="btnSaveRegistroDate">Guardar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    <?php if ($mlbInlineFechaEditEnabled): ?>
    (function () {
        var editRegistroDateModalEl = document.getElementById('editRegistroDateModal');
        var editRegistroDateModal = editRegistroDateModalEl ? new bootstrap.Modal(editRegistroDateModalEl) : null;
        var activeRegistroDateCell = null;

        $(document).on('click', '.js-edit-registro-date, .mlb-registro-date-hint', function (e) {
            e.preventDefault();
            e.stopPropagation();
            var $cell = $(this).closest('td.js-edit-registro-date');
            if (!$cell.length) {
                $cell = $(this).filter('.js-edit-registro-date');
            }
            if (!$cell.length) {
                return;
            }
            activeRegistroDateCell = $cell[0];
            var kind = String($cell.attr('data-lead-kind') || 'lead');
            $('#editRegistroDateKind').val(kind);
            $('#editRegistroDateTablaOrigen').val($cell.attr('data-tabla-origen') || '');
            $('#editRegistroDateLeadId').val($cell.attr('data-lead-id') || '0');
            $('#editRegistroDateContactFormId').val($cell.attr('data-contact-form-id') || '0');
            $('#editRegistroDateEventId').val($cell.attr('data-event-id') || '0');
            $('#editRegistroDateFecha').val($cell.attr('data-fecha') || '');
            $('#editRegistroDateHora').val($cell.attr('data-hora') || '');
            $('#editRegistroDateLeadName').text($cell.attr('data-lead-name') || 'Lead');
            if (kind === 'eventos_wp') {
                $('#editRegistroDateContext').text('Evento de Wedding Planner: actualiza eventos_wp.fecha_registro (y contact_form si aplica). No es el mismo campo que un lead en su tabla de origen.');
            } else {
                $('#editRegistroDateContext').text('Lead comercial: actualiza created_time en la tabla de origen del lead y sincroniza contact_form si existe.');
            }
            if (editRegistroDateModal) {
                editRegistroDateModal.show();
            }
        });

        $('#btnSaveRegistroDate').on('click', function () {
            var fecha = $('#editRegistroDateFecha').val();
            if (!fecha) {
            ...
            $.ajax({
                url: 'actualizar_fecha_registro_lead.php',
                type: 'POST',
                dataType: 'json',
                data: {
                    kind: $('#editRegistroDateKind').val(),
                    tabla_origen: $('#editRegistroDateTablaOrigen').val(),
                    lead_id: $('#editRegistroDateLeadId').val(),
                    contact_form_id: $('#editRegistroDateContactFormId').val(),
                    event_id: $('#editRegistroDateEventId').val(),
                    fecha: fecha,
                    hora: $('#editRegistroDateHora').val()
                },
                success: function (resp) {
                    if (resp && resp.success) {
                        if (activeRegistroDateCell && resp.display) {
                            var $cell = $(activeRegistroDateCell);
                            var $row = $cell.closest('tr');
                            $cell.empty();
                            $cell.append($('<span class="td-inline-muted"></span>').text(resp.display));
                            $cell.append($('<span class="mlb-registro-date-hint">Editar</span>'));
                            $cell.removeClass('is-empty');
                            if (resp.data_order) {
                                $cell.attr('data-order', resp.data_order);
                            }
                            $cell.attr('data-fecha', fecha);
                            $cell.attr('data-hora', $('#editRegistroDateHora').val() || '');
                            $row.find('.td-sub-fecha-registro').text(resp.display);
                            if (typeof leadsTableDT !== 'undefined' && leadsTableDT) {
                                leadsTableDT.row($row).invalidate();
                            }
                        }
                        if (editRegistroDateModal) {
                            editRegistroDateModal.hide();
                        }
    ...
    $(document).ready(function () {
        ...
        var leadsChartEl = document.getElementById('leadsChart');
        if (leadsChartEl && typeof Highcharts !== 'undefined') {
