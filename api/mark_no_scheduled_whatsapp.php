...
// Marks a lead as "estatus_whatsapp = 2" to indicate they did not schedule an appointment
...
try {
    ...
    // ========================================================================
    // VERIFICAR QUE EXISTE LA COLUMNA estatus_whatsapp EN LA TABLA
    // ========================================================================
    $columnExists = false;
    $checkColumn = $conn->query("SHOW COLUMNS FROM `" . $conn->real_escape_string($foundLeadTable) . "` LIKE 'estatus_whatsapp'");
    if ($checkColumn && $checkColumn->num_rows > 0) {
        $columnExists = true;
    }
    
    $debug['column_estatus_whatsapp_exists'] = $columnExists;
    
    // Si la columna no existe, intentar crearla
    if (!$columnExists) {
        $alterQuery = "ALTER TABLE `" . $conn->real_escape_string($foundLeadTable) . "` ADD COLUMN `estatus_whatsapp` TINYINT(3) DEFAULT 0";
        ...
        if ($alterResult) {
            ...
            $response['success'] = false;
            $response['message'] = 'No se pudo crear la columna estatus_whatsapp en la tabla ' . $foundLeadTable . ': ' . $conn->error;
            $response['debug'] = $debug;
            echo json_encode($response, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
            exit;
        }
    ...
    // ========================================================================
    // ACTUALIZAR EL CAMPO estatus_whatsapp = 1
    // ========================================================================
    ...
    $updateStmt = $conn->prepare("UPDATE `" . $foundLeadTable . "` SET estatus_whatsapp = 3 WHERE id = ?");
