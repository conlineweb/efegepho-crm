...
function isValidTimezoneName($timezoneName) {
...
function timezoneNameFromOffsetMinutes($offsetMinutes) {
    $offsetMinutes = (int)$offsetMinutes;
    $utcOffsetMinutes = -$offsetMinutes; // JS getTimezoneOffset: local = UTC - offset
    $sign = $utcOffsetMinutes >= 0 ? '+' : '-';
    $absMinutes = abs($utcOffsetMinutes);
    $hours = str_pad((string)floor($absMinutes / 60), 2, '0', STR_PAD_LEFT);
...
function urlHasTrackingParams($url) {
    ...
    foreach ($params as $key => $value) {
        ...
        if (strpos($normalizedKey, 'utm_') === 0 || in_array($normalizedKey, $trackingKeys, true)) {
            return true;
        }
    }
...
$sellerBaseTimezone = 'Etc/GMT+6'; // UTC-6 fijo como hora base operativa de la vendedora
...
if (count($vendedores) > 0) {
    ...
    $tracking_source_url = trim($_POST['tracking_source_url'] ?? '');
    $current_page_url = trim($_POST['current_page_url'] ?? '');

    if ($tracking_source_url !== '' && strcasecmp($tracking_source_url, 'direct') !== 0) {
        $trackingHasParams = urlHasTrackingParams($tracking_source_url);
        $referrerHasParams = urlHasTrackingParams($referrer_url);
        $shouldPromoteTracking = $referrer_url === '' || isBareOriginUrl($referrer_url) || $trackingHasParams || !$referrerHasParams;
        if ($shouldPromoteTracking) {
            $referrer_url = $tracking_source_url;
        }
    }

    if ($referrer_url !== '' && $current_page_url !== '') {
        $normalizedReferrer = rtrim($referrer_url, '/');
        $normalizedCurrent = rtrim($current_page_url, '/');
        if (strcasecmp($normalizedReferrer, $normalizedCurrent) === 0) {
            $referrer_url = '';
        }
    }

    // No atribuir el origen a la misma pantalla de inquire.
    if ($referrer_url !== '' && preg_match('#/inquire(?:/|\?|$)#i', $referrer_url)) {
        $referrer_url = '';
    }
    ...
    $timezone_name_raw = trim($_POST['timezone_name'] ?? '');
    $timezone_name = isValidTimezoneName($timezone_name_raw) ? $timezone_name_raw : '';
    $timezone_offset_min = intval($_POST['timezone_offset_min'] ?? 0); // Valor tal como viene de JS getTimezoneOffset()
    if ($timezone_name === '' && $timezone_name_raw !== '') {
        error_log('timezone_name inválida recibida: ' . $timezone_name_raw . '. Se aplicará fallback por offset.');
    }
    ...
    if ($from_website_form) {
        ...
        // Ajustar a la TZ real del cliente (IANA) o fallback por offset si viene sin IANA válida
        if ($timezone_name !== '') {
            $created_time_utc->setTimezone(new DateTimeZone($timezone_name));
        } else {
            $created_time_utc->setTimezone(new DateTimeZone(timezoneNameFromOffsetMinutes($timezone_offset_min)));
        }
    ...
    ensureColumnExists($conn, 'contact_form', 'referrer_url', "`referrer_url` VARCHAR(1000) DEFAULT ''");
    ...
    // Calcular UTC de la cita usando una única hora base operativa (vendedora: UTC-6 fijo).
    $appointment_utc = null;
    $date_time_str = trim($date_appointment . ' ' . $time_appointment);
    try {
        $dt = new DateTime($date_time_str, new DateTimeZone($sellerBaseTimezone));
        $dt->setTimezone(new DateTimeZone('UTC'));
        $appointment_utc = $dt->format('Y-m-d H:i:s');
        error_log("appointment_utc calculado ({$sellerBaseTimezone}): {$date_time_str} -> UTC={$appointment_utc}");
    } catch (Exception $e) {
    ...
    if ($zona_horaria_manual === '1') {
        ...
        try {
            ...
            if ($timezone_name !== '') {
                ...
                // Fallback: sin IANA, usar offset recibido por el dispositivo
                $offsetMinutes = (int)$timezone_offset_min;
                $fallbackTimezone = timezoneNameFromOffsetMinutes($offsetMinutes);
                $dtClient = clone $dtUtc;
                $tzClient = new DateTimeZone($fallbackTimezone);
                $dtClient->setTimezone($tzClient);
                error_log("Usando offset fallback para cliente local: offset_min={$offsetMinutes}, tz={$fallbackTimezone}, local=" . $dtClient->format('Y-m-d H:i:s'));
            }
