...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT c.*, cf.zona_horaria FROM calendario c LEFT JOIN contact_form cf ON c.idclie = cf.id WHERE c.id = ?";
...
<style>
    ...
    * {
    ...
    select::after {
        ...
        right: 10px;
        pointer-events: none;
        /* Evita que interfiera con el clic */
    }
    ...
    @media (max-width: 480px) {
        ...
        #country_code {
            ...
            max-width: 100%;
            /* Asegura que no se expanda más allá del contenedor */
            overflow-x: auto;
            /* Añade un desplazamiento horizontal si el contenido es demasiado largo */
        }
...
<body>
    <div class="form-container my-5">
        <?php if (!$mostrarMensaje && $mostrarFormulario): ?>
            <!-- Mostrar formulario cuando se encuentra el registro -->
            <form id="rescheduleForm">
                <center>
                    <div style="width:100%">
                        <h3 class="text-center mb-5">Reschedule a call with one of our consultants to receive a quote</h3>
        ...
        <?php elseif ($mostrarMensaje && !$mostrarFormulario): ?>
            <!-- Mostrar mensaje cuando no se encuentra el registro -->
            <center>
                <p>No existe el registro solicitado.</p>
            </center>
        <?php else: ?>
            <!-- Mostrar el formulario alternativo si no se encuentra el registro y no hay mensajes -->
            <form id="weddingForm" class="my-5 py-5">
                <input type="text" name="names" placeholder="your names">
                <input type="text" name="wedding_location" placeholder="wedding location">
                <input type="email" name="email" placeholder="email address">

                <!-- Cierre de la fila de la sección de país y fecha -->


                <div class="d-flex flex-row justify-content-between" style="width:100%">
                    <div class="d-flex flex-column" style="width:50%">
                        <label for="wedding_date">Wedding Date</label>
                        <input type="date" class="wedding_date" id="wedding_date" name="wedding_date"
                            onfocus="(this.type='date')" onblur="(this.type='text')">
                    </div>

                    <div class="d-flex flex-column" style="width:50%;padding-left: 10px;">
                        <label>Choose your country code</label>
                        <select id="country_code" name="country_code">
                            <option value="" disabled selected>Select your country code
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                &#709;</option>
                ...
                <input type="text" name="wedding_planner" placeholder="wedding planner">
                <input type="number" name="guests" pattern="[0-9]{10}" placeholder="how many guests will there be?">
                <input type="text" name="meet" placeholder="how did you meet?">
                <input type="text" name="couple_activities" placeholder="what do you like to do as a couple?">
                <input type="text" name="favorite_movie_song"
                    placeholder="do you have a favorite movie or song as a couple?">
                <input type="text" name="photo_style" placeholder="do you like a documentary or editorial look?">
                <!--<input type="text" name="how_did_hear" placeholder="how did you hear about us?">-->
                ...
                <input type="text" name="instagram" placeholder="can you share your Instagram handles with us?">
                ...
                <textarea name="details"
                    placeholder="we would love to hear about your plans in as much detail as possible."></textarea>

                <div style="width:100%">
                    <h3 class="text-center m-5">Schedule a call with one of our consultants to receive a quote</h3>
                    <div class="mt-1" id="meet">
                        <div class="calendar-container">
                            <div class="left-panel">
                                <div class="calendar-header">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        //  Swal.fire({
        //                         icon: 'success',
        //                         title: "Cita agendada",  // Muestra el mensaje principal
        //                         html: `Vendedor: <br>Fecha: <br>Hora: <br><br><h3></h3>`, // Muestra la fecha, hora, nombre del vendedor y el último mensaje
        ...
        function validateInput(input) {
            const validOptions = Array.from(input.list.options).map(option => option.value);
            if (!validOptions.includes(input.value)) {
                input.setCustomValidity('Por favor, seleccione una opción válida del listado.');
            } else {
        ...
        //este es el form para reagendar

        const formReschedule = document.getElementById("rescheduleForm");
        if (formReschedule) {
            const privacyCheckboxRes = formReschedule.querySelector('#privacyConsentReschedule');
            const resBtn = formReschedule.querySelector('#rescheduleButton');
            if (privacyCheckboxRes && resBtn) { resBtn.disabled = !privacyCheckboxRes.checked; privacyCheckboxRes.addEventListener('change', function () { resBtn.disabled = !this.checked; if (this.checked) $('#privacyConsentRescheduleError').hide(); else $('#privacyConsentRescheduleError').show(); }); }
        

            const idVendedor = <?php echo $idVendedor; ?>;

            var registro = <?php echo $registroJSON ?? 'null'; ?>;
            ...
            function generateCalendar(month, year) {
                ...
                minDate.setDate(today.getDate() - 1); // Fecha mínima (ayer)

                const maxDate = new Date(today);
                maxDate.setDate(today.getDate() + 14); // Fecha máxima (hoy + 14 días)
            ...
            async function showSchedule(event) {
                ...
                if (schedule && schedule.length > 0) {
                    ...
                    const clientTimezone = registro.zona_horaria || -6; // Zona horaria del cliente
                    const dif = local + clientTimezone;
            ...
            async function getSchedule(year, month, day) {
                ...
                // console.log("getSchedule day ",formattedDay)
                ...
                formData.append('idusu', idVendedor);
                let horas = [];
            ...
            formReschedule.addEventListener("submit", function (event) {
                event.preventDefault();
                if (!$('#privacyConsentReschedule').is(':checked')) { $('#privacyConsentRescheduleError').show(); Swal.fire({ icon: 'error', title: 'Please accept Terms & Conditions and Privacy Policy', customClass: { popup: 'swal2-popup-center' } }); return; }
                // Recolectar los datos del formulario
                const formData = new FormData(formReschedule); // Usar formReschedule aquí
            });
            ...
            formReschedule.addEventListener("submit", async function (event) {
                event.preventDefault();
                ...
                // Recolectar los datos del formulario
                const formData = new FormData(formReschedule);
                ...
                formData.append('timezone_name', confirmedTZ.timezone_name || registro.timezone_name || '');
                formData.append('timezone_offset_min', (confirmedTZ.timezone_offset_min ?? 0));
                formData.append('zona_horaria', (confirmedTZ.timezone_offset_hours ?? registro.zona_horaria ?? 0));
                const formDataObj = {};
                ...
                var data = {
                    id: registro.id,
                    vendedor: registro.idusu,
                    fecha: formDataObj.date_appointment,
                    hora: formDataObj.time_appointment,
                    cliente: registro.idclie,
                    hasSeller: true

                };

                let dataVendedor = {
                    date_appointment: data.fecha,
                    time_appointment: data.hora
                }
                ...
                //ajax
                $.ajax({
                    url: 'obtener-vendedor.php',
                    type: 'POST',
                    data: dataVendedor,
                    success: function (response) {
                        if (response.success) {
                            data.vendedor = response.vendedor;
                            console.log("data ", data);

                            $.ajax({
                                url: 'admin/reagendar.php', // Ruta al archivo PHP
                                type: 'POST',
                                data: data,
                                success: function (response) {
                                    loadingSwal.close();
                            ...
                            Swal.fire({
                                icon: 'warning',
                                title: 'No hay vendedores disponibles. Seleccione otro horario.',
                                confirmButtonText: 'Aceptar',
                                customClass: {
                                    popup: 'swal2-popup-center'
                                }
        ...
        if (weddingForm) {
            ...
            function validateWeddingForm() {
                let isValid = true;
                const requiredFields = weddingForm.querySelectorAll('input[required], select[required], textarea[required]');
                console.log(requiredFields)
                requiredFields.forEach(field => {
                    if (!field.value.trim()) {
                        // Aplicar estilos de error solo cuando se valida al hacer submit
                        field.style.borderBottom = '1px solid red';

                        isValid = false;
                    } else {
                        // Solo quitar los estilos si ya estaban mostrando error
                        if (field.style.borderBottom === '1px solid red') {
