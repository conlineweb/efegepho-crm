...
if (isset($_GET['source'])) {
    if ($_GET['origin'] == 'ig') {
        $origen = 2;
    } elseif ($_GET['source'] == 'whtsp') {
        $origen = 3;
    } elseif ($_GET['source'] == 'email') {
        $origen = 4;
    } else {
        $origen = 0; //default
    }
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (weddingForm) {
            ...
            form.addEventListener("submit", async function (event) {
                ...
                // Ubicación requerida para confirmar zona horaria real (pedirla inmediatamente)
                ...
                // Log vendor and client times to console (no DB writes)
                const vendorTime = $('li.selected').data('hour') || null;
                const clientTime = $('li.selected').data('hourcust') || null;
                console.log('[DEBUG] Vendor time (Mexico City/server):', vendorTime);
                console.log('[DEBUG] Client time (Local):', clientTime);
                console.log('[DEBUG] Selected date (server):', selecteddate);
                console.log('[DEBUG] Client adjusted date:', dateCustFormatted);

                // If debug_preview=1 in URL, do not submit to server — only show logs
                const debugNoSave = new URLSearchParams(window.location.search).get('debug_preview') === '1';
                if (debugNoSave) {
                    Swal.fire({
                        icon: 'info',
                        title: 'Debug mode: no database write',
                        html: `Vendor time: <strong>${vendorTime || 'N/A'}</strong><br>Client time: <strong>${clientTime || 'N/A'}</strong><br>Date (server): <strong>${selecteddate}</strong><br>Date (client): <strong>${dateCustFormatted}</strong>`,
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        // Normalize response: some servers may return a string (HTML/warnings) instead of JSON
                        let serverData = data;
                        if (typeof serverData === 'string') {
                            try {
                                ...
                                Swal.fire({
                                    ...
                                    html: `<pre style="text-align:left;white-space:pre-wrap;">${serverData}</pre>`,
                                    icon: "error",
                                    confirmButtonText: "OK",
                                    customClass: { popup: 'swal2-popup-center' }
                                });
