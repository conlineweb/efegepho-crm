...
try {
    ...
    // Verificar si ya existe un registro con el id_clie
    $stmt_check = $conn->prepare("SELECT COUNT(*) FROM respuestas_cuestionario WHERE id = ?");
    ...
    if ($count > 0) {
        // Si el registro con id_clie existe, lo actualizamos
      $stmt = $conn->prepare("UPDATE respuestas_cuestionario SET respuestas = ?, fecha = ?, hora = ?, estatus = 2 WHERE id = ?");
       ...
       $stmt = $conn->prepare("INSERT INTO respuestas_cuestionario (respuestas, id_clie, fecha, hora, estatus) VALUES (?, ?, ?, ?, 2)");
    $stmt->bind_param("siss", $respuestas_json, $idclie, $fecha_actual, $hora_actual);
